...
<%
    Application interviewApplication = (Application) request.getAttribute("application");
...
<div class="container">
    <div class="nav top-nav">
        ...
        <div class="user user-inline-actions"><span><%= escHtml(session.getAttribute("realName")) %> |</span><form action="<%= ctx %>/logout" method="post" class="inline-form logout-form"><%@ include file="/WEB-INF/jspf/csrf-hidden.jspf" %><button type="submit" class="logout-button">Logout</button></form></div>
    </div>
    <div class="page-layout">
        ...
        <main class="main-panel ta-main">
            ...
            <div class="context-card">
                <strong><%= escHtml(job != null ? job.getTitle() : interviewApplication.getJobId()) %></strong>
            ...
            <% if (interviewApplication.getInterviewSlotId() != null && !interviewApplication.getInterviewSlotId().isEmpty()) { %>
            <section class="detail-card">
                <h2>Current booking</h2>
                <p><strong>Time:</strong> <%= escHtml(interviewApplication.getInterviewTime() != null ? interviewApplication.getInterviewTime() : "-") %></p>
                <p><strong>Location:</strong> <%= escHtml(interviewApplication.getInterviewLocation() != null ? interviewApplication.getInterviewLocation() : "-") %></p>
                <form action="<%= ctx %>/ta/interview-booking" method="post" class="inline-form">
                    <%@ include file="/WEB-INF/jspf/csrf-hidden.jspf" %>
                    <input type="hidden" name="applicationId" value="<%= escHtml(interviewApplication.getId()) %>">
                ...
                <% if (interviewApplication.getInterviewTime() != null && !interviewApplication.getInterviewTime().trim().isEmpty()) { %>
                <span class="muted-inline"> | </span><a href="<%= ctx %>/ta/interview-calendar?applicationId=<%= escHtml(interviewApplication.getId()) %>">Download .ics</a>
                <% } %>
            </section>
            <% } %>
            ...
            <section class="detail-card">
                ...
                <div class="ta-duty-board">
                    <% for (SlotSummary summary : slotSummaries) {
                           ...
                           boolean bookedHere = summary.getSlot().getId().equals(interviewApplication.getInterviewSlotId());
                    %>
                    <article class="ta-duty-card">
                        ...
                        <form action="<%= ctx %>/ta/interview-booking" method="post">
                            ...
                            <button type="submit" class="btn btn-primary btn-sm"><%= interviewApplication.getInterviewSlotId() != null && !interviewApplication.getInterviewSlotId().isEmpty() ? "Change to this slot" : "Book this slot" %></button>
                        </form>
                        <% } %>
                    </article>
                    <% } %>
